...
# 	''' Operand will be either a register, an immediate, a register deref or an immediate deref '''
# 	if re.match(r'[A-Z]+', operand):
# 		# simple register
# 		return 'm_Registers.{:s}'.format(operand)
# 	elif re.match(r'[adr]\d{1,2}', operand):
...
with open('CallbackTable.txt', 'wt') as fh:
	for ix,i in enumerate(ins):
		...
		if len(d['operands']) > 0:
			opstring += ' ' + ' '.join(d['operands'])
		
		fh.write('\t{{{:s}, {{"{:s}", &Cpu::{:s}, {:s}, {:s}, {:s}}}}},\n'.format(hex(ix), opstring, d['methname'], d['width'], d['cycles'][0], d['cycles'][1]))
